...
#let resolve-cover-path-for(cover, page-path: "") = {
  let source = extract-cover-source(cover)
...
#let resolve-cover-path(cover) = {
  ...
  resolve-cover-path-for(cover, page-path: page-path)
}

#let resolve-public-cover-path(cover) = {
  let page-path = str(query-input("public-page-path", default: query-input("page-path", default: ""))).trim("/")
  resolve-cover-path-for(cover, page-path: page-path)
}
...
#let normalize-post-cover(cover, alt: none) = {
  ...
  if source == none or str(source) == "" {
    ...
    (
      ...
      public-source-path: resolve-public-cover-path(source),
  ...
  if normalized-cover == none {
    ...
    html-guard(() => {
      html.div(class: "post-cover", {
        html.elem("img", attrs: (
          src: normalized-cover.at("public-source-path"),
