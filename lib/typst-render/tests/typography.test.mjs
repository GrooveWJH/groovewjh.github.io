...
test('typst preview defines an explicit title and heading weight hierarchy', () => {
  const poemFrameTyp = readProjectFile('lib/typst-render/post/poem-frame.typ');
  const templatePostTyp = readProjectFile('lib/typst-render/post/template-post.typ');
...
test('system theme css keeps color-scheme separate from token themes and narrows runtime transition scope', () => {
  ...
  assert.match(
    lightCss,
    /html:not\(\[data-color-scheme\]\),\s*html\[data-color-scheme="light"\]\s*\{\s*color-scheme:\s*light;/s,
  );
  assert.match(lightCss, /html\[data-color-scheme="dark"\]\s*\{\s*color-scheme:\s*dark;/s);
  assert.match(
    systemDarkCss,
    /@media \(prefers-color-scheme: dark\)\s*\{\s*html:not\(\[data-color-scheme\]\)\s*\{\s*color-scheme:\s*dark;/s,
  );
  assert.match(
    systemDarkCss,
  ...
  assert.match(headerChromeCss, /header \.nav-title > \.nav-title-link\s*\{[^}]*color:\s*var\(--text-primary\);/s);
  assert.doesNotMatch(headerChromeCss, /header \.nav-title > \.nav-title-link\s*\{[^}]*color:\s*inherit;/s);
