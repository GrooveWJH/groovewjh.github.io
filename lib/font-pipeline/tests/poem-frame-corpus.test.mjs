...
import { cleanupTempDir, ROOT_DIR } from '../../testing/typst.mjs';
import { collectPublishedPoemFrameCorpusText, extractPoemFrameTextFromSource } from '../corpus/poem-frame.mjs';
...
#poem-frame[
  ...
  assert.match(corpus, /尾生毁柱/);
  assert.match(corpus, /献给白云一片理所当然的天空/);
...
test('collectPublishedPoemFrameCorpusText includes prelude poem-frame characters used by the live article', () => {
  ...
  for (const char of '献片理诗掩毕五尾毁柱看狂') {
    assert.ok(corpus.includes(char), `Expected poem corpus to include poem title glyph "${char}".`);
  }
});
