...
test('build emits normalized local share metadata and non-inline post cover html', () => {
  ...
  const poemHead = extractHead(readOutputHtml(built.outDirPath, 'posts/poems/Echo之死/index.html'));
  ...
  assert.match(
    homeHtml,
    /<button[^>]*data-theme-preference="light"[^>]*>[\s\S]*?<span class="nav-theme-toggle-icon" aria-hidden="true" data-theme-icon="light">[\s\S]*?<svg[\s\S]*?<\/svg>[\s\S]*?<\/span>[\s\S]*?<\/button>/,
  );
  assert.match(
    homeHtml,
    /<button[^>]*data-theme-preference="dark"[^>]*>[\s\S]*?<span class="nav-theme-toggle-icon" aria-hidden="true" data-theme-icon="dark">[\s\S]*?<svg[\s\S]*?<\/svg>[\s\S]*?<\/span>[\s\S]*?<\/button>/,
  );
  assert.match(
    homeHtml,
    /<button[^>]*data-theme-preference="auto"[^>]*>[\s\S]*?<span class="nav-theme-toggle-icon" aria-hidden="true" data-theme-icon="auto">[\s\S]*?<svg[\s\S]*?<\/svg>[\s\S]*?<\/span>[\s\S]*?<\/button>/,
  );
  ...
  assert.match(
    droneHead,
    /<meta name="twitter:description" content="时间的路上你孤独吗，\s*尽头是如水的寂寞；\s*你的原初是谁，你的未来何在；\s*你的青春你的梦想这两片花翅膀啊。?">/,
  );
  ...
  assert.doesNotMatch(preludeHead, /<link rel="preload" href="\/assets\/fonts\/[^"]+\.woff2"/);
  assert.match(preludeHead, /<link rel="preload" href="\/assets\/fonts\/NotoSerifCJKsc-Regular\.otf"/);
  assert.match(preludeHead, /<link rel="preload" href="\/assets\/fonts\/NotoSerifCJKsc-Bold\.otf"/);
  assert.match(preludeHead, /<link rel="preload" href="\/assets\/fonts\/Kaiti\.ttf"/);
  assert.match(poemHead, /<link rel="preload" href="\/assets\/fonts\/Kaiti\.ttf"/);
  assert.ok(
    poemHead.indexOf('/assets/fonts/Kaiti.ttf') < poemHead.indexOf('/assets/fonts.css'),
    'Expected poem Kai preload to be emitted before fonts.css.',
  );
  assert.match(
    preludeHtml,
