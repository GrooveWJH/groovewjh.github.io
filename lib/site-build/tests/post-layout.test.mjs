...
import { cleanupTempDir, createTempDir, ROOT_DIR } from '../../testing/typst.mjs';
...
test('build emits nine post layout matrix pages', () => {
  ...
  for (const slug of [
    ...
    assert.ok(
      existsSync(join(built.outDirPath, '__test', 'post-layout-matrix', 'cases', slug, 'index.html')),
      `Expected case page to exist: ${slug}`,
    );
  }
...
test('post cover spacing uses one shared clearance token', () => {
  const css = readFileSync(join(ROOT_DIR, 'assets', 'custom.css'), 'utf8');

  assert.match(css, /--post-cover-clearance:\s*1\.25rem;/);
  assert.match(
    css,
    /article\.post-article\.has-cover\s*>\s*section\s*\{[^}]*margin-top:\s*var\(--post-cover-clearance\)/s,
  );
  assert.match(
    css,
    /article\.post-article\.has-cover\s+\.post-cover\s*\{[^}]*margin:\s*0 0 var\(--post-cover-clearance\)/s,
  );
  assert.match(
    css,
    /article\.post-article\.has-cover\s+\.post-cover\s*\+\s*:where\([^)]*p[^)]*\)\s*\{[^}]*margin-top:\s*0/s,
  );
});
