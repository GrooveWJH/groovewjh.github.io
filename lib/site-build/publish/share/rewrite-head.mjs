...
function renderManagedHeadTags(payload) {
  ...
  const encodeMetaMultilineContent = (value) =>
  ...
  if (payload.descriptionText) {
    lines.push(`<meta name="typ-blog:description-text" content="${encodeMetaMultilineContent(payload.descriptionText)}">`);
  }
...
function stripManagedHeadTags(headHtml) {
  return normalizeHeadWhitespace(
    String(headHtml || '')
      .replace(/<link\s+rel="canonical"[^>]*>\s*/gi, '')
      .replace(buildAttrMatcher('name', 'description'), '')
      .replace(buildAttrMatcher('name', 'typ-blog:description-text'), '')
