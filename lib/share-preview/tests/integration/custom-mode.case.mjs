...
test('getActiveEntry returns a synthetic custom entry in custom mode', () => {
  ...
  assert.equal(entry.descriptionText, '自定义描述');
...
test('getActiveEntry preserves manual description line breaks in custom mode while keeping collapsed meta text', () => {
  const libraryEntry = createPreludeEntry({
    description: '那时候，你就会明白， 一切我们爱过与恨过的， 其实并没有什么不同。',
    descriptionText: '那时候，你就会明白，\n 一切我们爱过与恨过的，\n 其实并没有什么不同。',
  });
  const state = createPreviewState({ pages: [libraryEntry] }, null);

  state.activeMode = 'custom';

  const seededEntry = getActiveEntry(state);
  assert.equal(state.customDraft.description, '那时候，你就会明白，\n一切我们爱过与恨过的，\n其实并没有什么不同。');
  assert.equal(seededEntry.description, '那时候，你就会明白， 一切我们爱过与恨过的， 其实并没有什么不同。');
  assert.equal(seededEntry.descriptionText, '那时候，你就会明白，\n一切我们爱过与恨过的，\n其实并没有什么不同。');

  state.customDraft.description = '第一行\n 第二行\n第三行';
  const editedEntry = getActiveEntry(state);

  assert.equal(editedEntry.description, '第一行 第二行 第三行');
  assert.equal(editedEntry.descriptionText, '第一行\n第二行\n第三行');
});
