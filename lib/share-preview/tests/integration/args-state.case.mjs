...
test('createPreviewState defaults to square canvas with description and qr enabled', () => {
  ...
  assert.equal(state.activeMode, 'library');
  ...
  assert.equal(typeof state.customDraft.title, 'string');
  assert.equal(typeof state.customDraft.siteName, 'string');
  assert.equal(typeof state.customDraft.url, 'string');
