...
import { LEGACY_PREVIEW_TOOL_REL_DIR, PREVIEW_TOOL_REL_DIR } from '../../build/writer.mjs';
...
test('share-card-preview script emits preview page and manifest for an existing build', () => {
  ...
  const previewIndexPath = join(built.outDirPath, PREVIEW_TOOL_REL_DIR, 'index.html');
  ...
  const versionFilePath = join(built.outDirPath, PREVIEW_TOOL_REL_DIR, 'version.json');
  const manifestPath = join(
    built.outDirPath,
    PREVIEW_TOOL_REL_DIR,
  ...
  assert.ok(existsSync(versionFilePath), 'Expected preview version file to exist');
  ...
  assert.match(previewIndexHtml, /data-preview-version="[^"]+"/);
  assert.match(previewIndexHtml, /data-preview-version-url="\.\/version\.json"/);
  assert.match(previewIndexHtml, /data-manifest-url="\.\/manifest\.[^"]+\.json"/);
  assert.match(previewIndexHtml, /<meta name="robots" content="noindex, nofollow">/);
  assert.match(previewIndexHtml, /http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate"/);
  ...
  assert.match(imessageRendererSource, /panelBg/);
  ...
  assert.equal(existsSync(join(built.outDirPath, LEGACY_PREVIEW_TOOL_REL_DIR, 'index.html')), false);
});
