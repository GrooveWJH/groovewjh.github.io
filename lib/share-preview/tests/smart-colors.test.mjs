...
import { analyzeImageSpatialData } from '../runtime/smart-color-analysis.mjs';
import {
...
test('resolveSmartPreviewColorsFromPalette picks a dark panel for top-heavy space-like covers', () => {
  const dark = [11, 14, 20];
  const mid = [58, 78, 110];
  const low = [86, 101, 126];
  const spatial = analyzeImageSpatialData(
    createImageData([
  ...
  const result = resolveSmartPreviewColorsFromPalette({
    dominant: mid,
    palette: [mid, low, dark, [185, 203, 226]],
    spatial,
  });

  assert.equal(result.toneMode, 'dark-panel');
  assert.match(result.selectionReason, /深色锚点|整体情绪/);
});

test('resolveSmartPreviewColorsFromPalette keeps airy covers on a light panel and supports dark intent override', () => {
  const pale = [234, 236, 240];
  const cream = [244, 241, 233];
  const dark = [28, 37, 51];
  const lightSpatial = analyzeImageSpatialData(
    createImageData([
  ...
  const automatic = resolveSmartPreviewColorsFromPalette({
    dominant: pale,
    palette: [pale, cream, dark],
    spatial: lightSpatial,
  });
  const overridden = resolveSmartPreviewColorsFromPalette({
    dominant: pale,
    palette: [pale, cream, dark],
    panelIntent: 'dark-anchor',
  });

  assert.equal(automatic.toneMode, 'light-panel');
  assert.equal(overridden.toneMode, 'dark-panel');
  assert.equal(overridden.panelIntent, 'dark-anchor');
  assert.ok(overridden.selectionReason.length > 0);
});
