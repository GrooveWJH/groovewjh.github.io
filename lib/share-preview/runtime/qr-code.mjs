...
export function renderQrOverlay(geometry, smartColors = null) {
  ...
  return `<g data-part="imessage-qr-overlay" aria-label="iMessage 二维码覆盖层">
    <rect x="${geometry.x}" y="${geometry.y}" width="${geometry.size}" height="${geometry.size}" rx="${geometry.radius}" ry="${geometry.radius}" data-part="imessage-qr-mask" aria-label="iMessage 二维码底板" fill="transparent"></rect>
