import { normalizeSmartColorTokens } from './smart-colors-core.mjs';
...
export function deriveQrColors(smartColors = null) {
  const colors = normalizeSmartColorTokens(smartColors);
  return {
    overlayColor: colors.qrMaskBg,
    symbolColor: colors.qrFg,
  };
}
...
export function renderQrOverlay(geometry, smartColors = null) {
  ...
  return `<g data-part="imessage-qr-overlay" aria-label="iMessage 二维码覆盖层">
    <rect x="${geometry.x}" y="${geometry.y}" width="${geometry.size}" height="${geometry.size}" rx="${geometry.radius}" ry="${geometry.radius}" data-part="imessage-qr-mask" aria-label="iMessage 二维码底板" fill="${colors.overlayColor}"></rect>
    <path d="${geometry.path}" data-part="imessage-qr-symbol" aria-label="iMessage 二维码图形" fill="${colors.symbolColor}"></path>
  </g>`;
}
