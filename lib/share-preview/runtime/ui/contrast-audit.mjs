...
export function buildContrastAuditModel({
  ...
  const results = TARGET_DEFINITIONS.map((definition) =>
    buildAuditResult(definition, safeDisplayOptions, parts, smartColors),
  );
  ...
  return {
    ...
    summary: {
      ...
      rolePassCount: groupSummaries.filter((group) => group.aaPlusPass).length,
      roleTotalCount: groupSummaries.length,
      groupSummaries,
    },
