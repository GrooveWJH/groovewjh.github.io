...
function formatPayloadRows(entry, smartColors) {
  return [
    ...
    ['panelIntent', entry.panelIntent || 'auto'],
    ...
    ['resolved.panelBg', smartColors?.panelBg || '—'],
    ['resolved.toneMode', smartColors?.toneMode || '—'],
    ['resolved.selectionReason', smartColors?.selectionReason || '—'],
  ];
}
...
function renderEdgeFeatherControl(edgeFeatherEnabled, edgeFeatherSize) {
  const safeSize = Number.isFinite(Number(edgeFeatherSize)) ? Number(edgeFeatherSize) : 2.4;
  return `<fieldset class="carbon-preview-option-group">
    <legend class="carbon-preview-option-group__label">${renderIcon('preview')}<span>边缘羽化</span></legend>
    <div class="carbon-preview-option-stack">
      <label class="carbon-preview-toggle">
        <input type="checkbox" data-edge-feather-toggle ${edgeFeatherEnabled ? 'checked' : ''}>
        ${renderIcon('preview')}
        <span class="carbon-preview-toggle__label">启用羽化</span>
      </label>
      <div class="carbon-preview-range-control">
        <label class="carbon-preview-range-control__label" for="edge-feather-size-input">羽化强度</label>
        <div class="carbon-preview-range-control__row">
          <input
            id="edge-feather-size-input"
            class="carbon-preview-range-control__input"
            type="range"
            min="0"
            max="8"
            step="0.2"
            value="${safeSize}"
            data-edge-feather-size-input
            ${edgeFeatherEnabled ? '' : 'disabled'}
          >
          <output class="carbon-preview-range-control__value" for="edge-feather-size-input">${safeSize.toFixed(1)}</output>
        </div>
      </div>
    </div>
  </fieldset>`;
...
function renderTitleOptionList(displayOptions) {
  const checked = Boolean(displayOptions?.brackets);

  return `<fieldset class="carbon-preview-option-group">
    <legend class="carbon-preview-option-group__label">${renderIcon('title')}<span>标题样式</span></legend>
    <div class="carbon-preview-option-list">
      <label class="carbon-preview-segment">
        <input type="checkbox" data-display-option="brackets" ${checked ? 'checked' : ''}>
        ${renderIcon('title')}
        <span>保留 ｢ ｣</span>
      </label>
    </div>
...
export function renderDisplayOptions(entry, displayOptions, cornerSize, edgeFeatherEnabled, edgeFeatherSize) {
  const controls = [
  ...
  return `<div class="carbon-preview-option-stack">
    <fieldset class="carbon-preview-option-group">
    ...
    ${renderTitleOptionList(displayOptions)}
    ${renderCornerSizeControl(cornerSize)}
    ${renderEdgeFeatherControl(edgeFeatherEnabled, edgeFeatherSize)}
  </div>`;
}
...
export function renderPayloadPanel(entry, smartColors = null) {
  const preface = entry?.isCustom
    ? '<p class="carbon-preview-payload-note">当前 payload 来自自定义模式，仅用于本地预览与导出，不会写回站点构建结果。</p>'
    : '';

  return `${preface}<dl class="carbon-preview-payload-grid">${formatPayloadRows(entry, smartColors)
