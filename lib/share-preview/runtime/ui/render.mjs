...
import { syncCanvasToggleHost, syncDisplayOptionsHost } from './sync-controls.mjs';
...
export function renderPreview(nodes, state, filteredPages, activeEntry, runtimeMeta = null) {
  ...
  syncCanvasToggleHost(nodes.canvasToggleHost, state.squareCanvas);
  ...
  syncDisplayOptionsHost(
    nodes.displayOptionsHost,
    activeEntry,
    state.displayOptions,
    state.cornerSize,
    state.edgeFeatherEnabled,
    state.edgeFeatherSize,
  );
  ...
  nodes.payloadNode.innerHTML = renderPayloadPanel(activeEntry, smartColors);
  nodes.reviewNode.hidden = state.activeInspectorPanel !== 'review';
  nodes.payloadNode.hidden = state.activeInspectorPanel !== 'payload';
