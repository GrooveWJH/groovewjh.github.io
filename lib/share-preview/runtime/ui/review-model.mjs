...
export function buildReviewModel(entry, { colorSamplingState = 'unavailable', exportStatus = 'idle' } = {}) {
  if (!entry) {
    return null;
  }
  ...
  if (!entry.imageUrl && entry.imagePath) {
    diagnostics.push({
      ...
      icon: 'image',
      tone: 'warning',
      text: '当前预览可以使用本地图片路径渲染，但无法校验绝对分享图 URL。',
    });
  }
  ...
  if (exportStatus === 'busy') {
    ...
    diagnostics.push({
      ...
      icon: 'warning',
      tone: 'warning',
      text: '上一次 PNG 导出失败，请查看浏览器弹窗中的具体错误信息。',
    });
  }
