...
function resolveEdgeFeatherSize(value) {
  const numericValue = Number(value);
  return Number.isFinite(numericValue) ? Math.max(0, numericValue) : 2.4;
}
...
export function renderIMessageCard(entry, imageHref, options = {}) {
  ...
  const edgeFeatherSize = resolveEdgeFeatherSize(options.edgeFeatherSize);
  const showEdgeFeather = !options.squareCanvas && options.edgeFeatherEnabled !== false && edgeFeatherSize > 0;
  const outerCanvasBackground = options.squareCanvas
    ? `<rect width="600" height="${model.cardHeight}" fill="#edf1f7"></rect>`
    : '';
  const edgeFeatherFilter = !showEdgeFeather
    ? ''
    : `<filter id="imessage-edge-feather" x="-12%" y="-12%" width="124%" height="124%" color-interpolation-filters="sRGB">
        <feMorphology in="SourceAlpha" operator="dilate" radius="${edgeFeatherSize.toFixed(1)}" result="expanded-alpha"></feMorphology>
        <feGaussianBlur in="expanded-alpha" stdDeviation="${(edgeFeatherSize * 0.92).toFixed(1)}" result="expanded-blur"></feGaussianBlur>
  ...
  const edgeFeatherPath = !showEdgeFeather
    ? ''
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" data-part="imessage-card" aria-label="iMessage 分享卡片" viewBox="0 0 600 ${model.cardHeight}" width="600" height="${model.cardHeight}">
    ...
    <rect x="${CARD_X}" y="${model.infoY}" width="${model.cardWidth}" height="${model.infoHeight}" data-part="imessage-metadata-panel" aria-label="iMessage 信息面板" fill="${model.colors.panelBg}" clip-path="url(#imessage-card)"></rect>
