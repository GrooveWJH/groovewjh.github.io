...
export function createPreludeEntry({ includeDescription = true, includePublishedTime = true } = {}) {
  const descriptionMeta = includeDescription ? '<meta name="description" content="那时候，你就会明白，一切我们爱过与恨过的，其实并没有什么不同。">' : "";
...
export function getTextY(svg, part, x = null) {
  const prefix = x == null ? `<text[^>]*y="([0-9.]+)" data-part="${part}"` : `<text x="${x}" y="([0-9.]+)" data-part="${part}"`;
  const match = svg.match(new RegExp(prefix));
  return match ? Number(match[1]) : null;
}

export function getRectBox(svg, part) {
  const match = svg.match(new RegExp(`<rect x="([0-9.]+)" y="([0-9.]+)" width="([0-9.]+)" height="([0-9.]+)"[^>]*data-part="${part}"`));
