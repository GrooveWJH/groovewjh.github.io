...
import { buildTypstHtml, cleanupTempDir, compileTypstPdfText, compileTypstSvg } from './test-helpers/typst-test-helpers.mjs';
...
`;
  ...
  assert.equal(htmlCompiled.result.status, 0, `Expected html compile to succeed.\nstdout:\n${htmlCompiled.result.stdout}\nstderr:\n${htmlCompiled.result.stderr}`);
  ...
  assert.equal(previewCompiled.result.status, 0, `Expected typst svg compile to succeed.\nstdout:\n${previewCompiled.result.stdout}\nstderr:\n${previewCompiled.result.stderr}`);
  cleanupTempDir(previewCompiled.tempDir);

  const templatePostSource = readFileSync(join(ROOT_DIR, 'lib/typ2html/template-post.typ'), 'utf8');
  assert.match(templatePostSource, /#import "raw\.typ": normalize-raw-block-text/);
