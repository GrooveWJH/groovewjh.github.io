...
test('theme toggle uses Carbon-like bare header actions with underline-only active state', () => {
  ...
  assert.match(layoutTyp, /html\.elem\("svg"/);
  assert.match(layoutTyp, /class: "nav-theme-toggle-icon"/);
  assert.match(layoutTyp, /data-theme-icon": preference/);
  ...
  assert.match(themeToggleCss, /header \.nav-theme-toggle-button\[data-theme-preference="auto"\] \.nav-theme-toggle-icon\s*\{[^}]*width:\s*1\.1875rem;[^}]*height:\s*1\.1875rem;/s);
  assert.match(themeToggleCss, /header \.nav-theme-toggle-icon svg\s*\{[^}]*fill:\s*currentColor;/s);
  assert.doesNotMatch(themeToggleCss, /mask-image:\s*url\("\/assets\/icons\/automatic\.svg"\)/);
