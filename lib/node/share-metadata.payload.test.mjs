...
import { ROOT_DIR, buildTypstHtml, extractHead, extractMetadataJson } from "./test-helpers/share-test-helpers.mjs";
...
`,
  ...
  assert.equal(stringCover.result.status, 0, `Expected metadata html compile to succeed.\nstdout:\n${stringCover.result.stdout}\nstderr:\n${stringCover.result.stderr}`);
...
`,
  ...
  assert.equal(imageCover.result.status, 0, `Expected metadata html compile for image-object cover to succeed.\nstdout:\n${imageCover.result.stdout}\nstderr:\n${imageCover.result.stderr}`);
