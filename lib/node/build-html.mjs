...
const TEMP_SLUGS_JSON_PATH = join(ROOT_DIR, ".typ-blog-cache", "_route-slugs.json");
const STAGING_SITE_DIR = join(ROOT_DIR, ".typ-blog-cache", "_site-staging");
...
function compileDynamicPageTypFilesToHtml(siteDir, force, typFiles, dependencyPaths, posts, slugsJsonPath) {
  const compiled = [];
  let failedCount = 0;
  ...
  for (const typFile of typFiles) {
    ...
    for (const target of targets) {
      ...
      const slugsJsonInput = relative(dirname(typFile), slugsJsonPath);
      const inputEntries = [
        ["page-path", pagePath],
        ["posts-json", postsJsonInput],
        ["slugs-json", slugsJsonInput],
      ];
...
function syncStagingSiteToOutput(stagingDir, outputDir) {
...
function collectRouteSlugs(posts) {
  const tags = Array.from(new Set(posts.flatMap((post) => post.tags))).sort((a, b) => a.localeCompare(b));
  const categories = Array.from(new Set(posts.map((post) => post.category).filter(Boolean))).sort((a, b) => a.localeCompare(b));

  return {
    tags: Object.fromEntries(tags.map((tag) => [tag, slugify(tag)])),
    categories: Object.fromEntries(categories.map((category) => [category, slugify(category)])),
  };
}
...
function writeTempSlugsFile(routeSlugs) {
  const json = JSON.stringify(routeSlugs, null, 2);
  const changed = writeFileIfChanged(TEMP_SLUGS_JSON_PATH, json);
  return {
    path: TEMP_SLUGS_JSON_PATH,
    changed,
  };
}
...
function main() {
  ...
  const outputSiteDir = join(ROOT_DIR, outDirName);
  const stagingSiteDir = STAGING_SITE_DIR;

  rmSync(stagingSiteDir, { recursive: true, force: true });
  mkdirSync(stagingSiteDir, { recursive: true });
  mkdirSync(outputSiteDir, { recursive: true });

  const assetsStats = copyAssetsToSite(stagingSiteDir, true);
  ...
  const postAssetStats = copySourceAssetsToSite(POSTS_DIR, stagingSiteDir, true, postAssetFiles);
  const pageAssetStats = copySourceAssetsToSite(PAGES_DIR, stagingSiteDir, true, pageAssetFiles);
  ...
  const routeSlugs = collectRouteSlugs(posts);
  const tempSlugs = writeTempSlugsFile(routeSlugs);
  ...
  const pageCompile = compileDynamicPageTypFilesToHtml(
    ...
    force,
    pageTypFiles,
    [tempMetadata.path, tempSlugs.path],
    posts,
    tempSlugs.path,
  );
  ...
  let syncedToOutput = false;
  let legacyPageCleanupStats = { removed: 0 };
  let obsoleteHtmlCleanupStats = { removed: 0 };
  let assetsCleanupStats = { removed: 0 };

  if (failedCount === 0) {
    legacyPageCleanupStats = cleanupLegacyFlatPageOutputs(outputSiteDir, pageTypFiles, posts);
    syncStagingSiteToOutput(stagingSiteDir, outputSiteDir);
    syncedToOutput = true;

    const expectedOutputPaths = compiled.map((item) => {
      const relOutputPath = relative(stagingSiteDir, item.output);
      return join(outputSiteDir, relOutputPath);
    });

    obsoleteHtmlCleanupStats = cleanupObsoleteGeneratedHtml(outputSiteDir, expectedOutputPaths);
    assetsCleanupStats = cleanupSiteAssets(outputSiteDir);
  }
  ...
  if (!syncedToOutput) {
    console.log("Output sync skipped due to compile errors");
  }
  ...
  console.log(`Route slugs json: ${normalizePosixPath(relative(ROOT_DIR, tempSlugs.path))} (${tempSlugs.changed ? "updated" : "unchanged"})`);
