...
import { PAGES_DIR, POSTS_DIR, ROOT_DIR, TYPE_TOOLCHAIN_DIR } from './build/constants.mjs';
...
const FONT_GENERATOR_VERSION = 'font-artifacts-v2';
...
const NON_BASIC_UNICODE_RANGE = 'U+0080-10FFFF';
...
function buildSubsetDescriptors(sourceFingerprints, siteCorpusText, poemCorpusText) {
  const sourceByPath = new Map(
    sourceFingerprints.map((fingerprint) => [join(ROOT_DIR, fingerprint.path), fingerprint]),
  );
...
export function buildFontCssText() {
  ...
  appendPoemFontAliasCss(lines, {
    ...
    nonBasicSrc:
      `url("/assets/fonts/site-kai-non-basic-400.woff2") format("woff2"), local("KaiTi"), ` +
