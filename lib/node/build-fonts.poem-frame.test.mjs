...
import { buildSubsetDescriptors } from './build-fonts.mjs';
import { collectPublishedPoemFrameCorpusText } from './poem-frame-corpus.mjs';
...
test('buildSubsetDescriptors feeds published poem-frame and poem header glyphs into Site Kai non-basic subset input', () => {
  ...
  for (const char of '献片理诗掩毕五尾毁柱看狂') {
    assert.ok(kaiNonBasic.textValue.includes(char), `Expected Site Kai subset text to include poem header glyph "${char}".`);
  }
});
