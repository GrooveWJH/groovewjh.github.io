...
test('html callouts render shared default titles when title is omitted', () => {
  const compiled = buildTypstHtml({
    sourceText: `#import "../config.typ": *
...
`,
    inputs: [
      ['page-path', 'posts/callout-default-title-check'],
    ],
  });
  ...
  assert.match(compiled.html, /<div class="note-block">[\s\S]*?<div class="block-title">提示<\/div>[\s\S]*?<p>默认 note<\/p>[\s\S]*?<\/div>/);
  assert.match(compiled.html, /<div class="success-block">[\s\S]*?<div class="block-title">完成<\/div>[\s\S]*?<p>默认 success<\/p>[\s\S]*?<\/div>/);
  assert.match(compiled.html, /<div class="warning-block">[\s\S]*?<div class="block-title">警告<\/div>[\s\S]*?<p>默认 warning<\/p>[\s\S]*?<\/div>/);
  assert.match(compiled.html, /<div class="error-block">[\s\S]*?<div class="block-title">注意<\/div>[\s\S]*?<p>默认 error<\/p>[\s\S]*?<\/div>/);

  rmSync(compiled.tempDir, { recursive: true, force: true });
});

test('html callouts prefer explicit titles over default titles', () => {
  const compiled = buildTypstHtml({
    sourceText: `#import "../config.typ": *
...
`,
    inputs: [
      ['page-path', 'posts/callout-explicit-title-check'],
    ],
  });
  ...
  assert.match(compiled.html, /<div class="warning-block">[\s\S]*?<div class="block-title">注意！<\/div>[\s\S]*?<p>显式 warning<\/p>[\s\S]*?<\/div>/);
  assert.doesNotMatch(compiled.html, /<div class="block-title">警告<\/div>/);
