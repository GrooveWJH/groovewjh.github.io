...
// 读取文件 mtime（毫秒）；文件不存在时返回 0。
export function getMtimeMs(filePath) {
  try {
...
// 判断输出是否需要刷新（force / source / 依赖）。
export function shouldRefreshOutput(sourcePath, outputPath, dependencyPaths, force) {
  if (force || !existsSync(outputPath)) {
    return true;
  }

  const targetMtime = getMtimeMs(outputPath);
  if (getMtimeMs(sourcePath) > targetMtime) {
    return true;
  }
...
// 复制 source 或复用旧输出到 staging，并记录状态。
export function stageFileFromSourceOrOutput(sourcePath, outputPath, stagingPath, force, statusMap, relPath) {
  const shouldUpdate = force
    || !existsSync(outputPath)
    || getMtimeMs(sourcePath) > getMtimeMs(outputPath);
  ...
  cpSync(outputPath, stagingPath, { force: true });
  upsertStatus(statusMap, relPath, "unchanged");
}
