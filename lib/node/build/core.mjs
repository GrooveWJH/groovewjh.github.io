...
function makePageCompileTasks(pageTypFiles, posts, slugMaps, postsJsonPath, slugsJsonPath, dependencies, siteInputs = {}) {
  ...
  for (const typFile of pageTypFiles) {
    ...
    if (relTypPath === "index.typ") {
      appendIndexPaginationTasks(tasks, typFile, posts.length, shared);
      continue;
    }
...
function prepareBuildPaths({ outDirName, configPath, cacheRoot }) {
  ...
  return {
    paths: {
      ...
      stagingSiteDir: join(resolvedCacheRoot.resolvedPath, "_site-staging"),
      metadataCacheDir: join(resolvedCacheRoot.resolvedPath, "metadata"),
      postsMetadataJsonPath: join(resolvedCacheRoot.resolvedPath, "_posts-metadata.json"),
      routeSlugsJsonPath: join(resolvedCacheRoot.resolvedPath, "_route-slugs.json"),
    },
...
export async function buildSite({ outDirName, jobs, configPath, cacheRoot }) {
  ...
  const buildData = await collectBuildData(paths, jobs);

  // 3) 组装并校验编译任务。
  const allCompileTasks = buildCompileTaskSet(buildData, paths, siteInputs);
