    ...
    async (task) => {
      ...
      return {
        ...
        post: {
          ...
          lastModified: new Date(getMtimeMs(task.typFile)).toISOString(),
          source: normalizePosixPath(relative(ROOT_DIR, task.typFile)),
        },
      };
    },
    progressBar,
...
function makePostCompileTasks(postTypFiles, postsJsonPath, dependencies) {
  ...
  return postTypFiles.map((typFile) => {
    ...
    return {
      ...
      dependencies: [...dependencies, postsJsonPath],
      routeLabel: "",
    };
  });
}
...
function makePageCompileTasks(pageTypFiles, posts, slugMaps, postsJsonPath, slugsJsonPath, dependencies) {
  const postsInput = normalizePosixPath(relative(TYPE_TOOLCHAIN_DIR, postsJsonPath));
  ...
  for (const typFile of pageTypFiles) {
    ...
    if (usage.hasTag) {
      for (const tag of tags) {
        ...
        const pagePath = derivePagePathFromIndexTyp(PAGES_DIR, join(PAGES_DIR, relTarget));
        tasks.push({
          kind: "page",
          sourceTypFile: typFile,
...
// 仅编译需要刷新的任务，其余复用旧产物到 staging。
async function compileAndStageTypstTasks(tasks, outputSiteDir, stagingSiteDir, force, jobs, statusMap) {
  const needsCompile = [];

  for (const task of tasks) {
    const oldOutputPath = join(outputSiteDir, task.outputRel);
    const stagingOutputPath = join(stagingSiteDir, task.outputRel);

    const refresh = shouldRefreshOutput(task.sourceTypFile, oldOutputPath, task.dependencies, force);

    if (!refresh && existsSync(oldOutputPath)) {
      ensureDirForFile(stagingOutputPath);
      cpSync(oldOutputPath, stagingOutputPath, { force: true });
      upsertStatus(statusMap, task.outputRel, "unchanged");
      continue;
    }
  ...
  const progressBar = createProgressBar("compile ", needsCompile.length);
  const results = await runPool(
    needsCompile,
    jobs,
    async (task) => {
...
function stageAssetGroup(sourceRoot, sourceFiles, outputSiteDir, stagingSiteDir, targetPrefix, force, statusMap, label) {
  ...
  for (const sourcePath of sourceFiles) {
    ...
    const outputPath = join(outputSiteDir, outputRel);
    const stagingPath = join(stagingSiteDir, outputRel);

    stageFileFromSourceOrOutput(sourcePath, outputPath, stagingPath, force, statusMap, outputRel);
    if (progressBar) {
      progressBar.tick();
    }
  }
...
export async function buildSite({ outDirName, force, jobs, configPath, cacheRoot }) {
  ...
  const postCompileTasks = makePostCompileTasks(postSource.typFiles, paths.postsMetadataJsonPath, typstDependencies);
  ...
  await compileAndStageTypstTasks(allCompileTasks, outputSiteDir, paths.stagingSiteDir, force, jobs, statusMap);

  // 5) 合并静态资源（站点资源 / 文章资源 / 页面资源）。
  const siteAssets = walkFiles(ASSETS_DIR);
  stageAssetGroup(ASSETS_DIR, siteAssets, outputSiteDir, paths.stagingSiteDir, "assets", force, statusMap, "assets:site ");
  stageAssetGroup(POSTS_DIR, postSource.assetFiles, outputSiteDir, paths.stagingSiteDir, "posts", force, statusMap, "assets:post ");
  stageAssetGroup(PAGES_DIR, pageSource.assetFiles, outputSiteDir, paths.stagingSiteDir, "", force, statusMap, "assets:page ");
  ...
  console.log(`Compiled tasks: ${allCompileTasks.length}`);
  console.log(`✅ Updated: ${updatedCount} | ⚪ Unchanged: ${unchangedCount} | 🗑️ Deleted: ${deletedFiles.length}`);
  console.log(`📝 Posts: ${posts.length} | 🏷️ Tags: ${Object.keys(slugMaps.tags).length} | 📂 Categories: ${Object.keys(slugMaps.categories).length}`);
