...
import { ASSETS_DIR, PAGES_DIR, POSTS_DIR, ROOT_DIR, TYPE_TOOLCHAIN_DIR } from './constants.mjs';
import {
...
import {
  collectTypstDependencyFiles,
  makeTypstCompileArgs,
  parseJsonFromMetadataHtml,
  runTypstCompile,
} from './typst.mjs';
...
function stageStaticAssets(buildData, paths, statusMap) {
  const siteAssets = walkFiles(ASSETS_DIR, (filePath) => {
    ...
    return rel !== 'fonts.css' && !rel.startsWith('fonts/') && !WEBAPP_ICON_FILE_NAMES.includes(rel);
  });
