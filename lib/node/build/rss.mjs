...
function readSiteInfoFromExternalConfig(siteConfigPath) {
  ...
  return {
    ...
    siteUrl: String(parsed.siteUrl || DEFAULT_RSS_SITE_URL)
      .trim()
      .replace(/\/+$/, ''),
...
function buildRssXml(posts, siteInfo) {
  ...
  const lastBuildDate = latest ? toRssDate(latest.date, latest.lastModified) : new Date().toUTCString();
  ...
  const items = sortedPosts
    .map((post) => {
      ...
      const categoryNodes = [
        post.category ? `    <category domain="category">${escapeXml(post.category)}</category>` : '',
