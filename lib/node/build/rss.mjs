...
export function stageRss(posts, outputSiteDir, stagingSiteDir, siteConfigPath, statusMap) {
  ...
  if (existsSync(oldOutputPath) && safeRead(oldOutputPath) === rssXml) {
    cpSync(oldOutputPath, stagingPath, { force: true });
    upsertStatus(statusMap, outputRel, "unchanged");
    return;
  }

  writeFileSync(stagingPath, rssXml, "utf8");
  upsertStatus(statusMap, outputRel, "updated");
}
