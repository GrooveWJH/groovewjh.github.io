...
import { getDomainLabel, measureTextMetrics, measureTextWidth, splitDisplayTitle, wrapTextByWidth } from "../svg-utils.mjs";
...
function buildMetadataText(entry, displayOptions, layout, titleParts) {
  ...
  return {
    ...
    dateX: dateLines[0] ? layout.contentColumnX + ((siteNameLines[0] ? measureTextWidth(siteNameLines[0], { fontSize: 17, fontWeight: 600 }) : 0) + 18) : 0,
  };
}

export function resolveIMessageDescriptionBreakState(entry, { includeDescription = false, displayOptions = null } = {}) {
  const showDescription = includeDescription && displayOptions?.description !== false;
  if (!showDescription) {
...
export function buildIMessageCardModel(entry, { includeDescription = false, smartColors = null, displayOptions = null } = {}) {
  ...
  const textRequiredHeight = topContentHeight + (bottomMetaHeight > 0 ? 18 + bottomMetaHeight : 0) + layout.panelPadding.bottom;
  const infoHeight = Math.max(title.showDescription ? 148 : 112, textRequiredHeight, showQr ? layout.panelPadding.top + QR_SIZE + layout.qrRailInset : 0);
  const infoY = 18 + coverHeight;
  const qrGeometry = showQr ? buildQrGeometry(entry.qrUrl, { x: layout.qrFrame.x, y: infoY + infoHeight - QR_SIZE - layout.qrRailInset, size: layout.qrFrame.size }) : null;
  ...
  const metaRowBottomY = metadata.siteNameLines.length || metadata.dateLines.length ? (metadata.domainLines.length ? domainBottomY - 30 : metadataBottomY) : 0;
  return {
    ...
    qrRailAttributes: qrGeometry ? `data-part="imessage-qr-rail" aria-label="iMessage QR Rail" data-layout-left="${layout.contentColumnRight + layout.qrGutter}" data-layout-width="${layout.qrRailWidth - layout.qrGutter}"` : "",
    title: buildTextModel(title.titleLines, layout.contentColumnX, infoY + layout.panelPadding.top, title.titleFontSize, 24, 700, colors.titleColor, 'data-part="imessage-title-block" aria-label="iMessage Title Block"'),
    description: buildTextModel(title.descriptionLines, layout.contentColumnX, infoY + layout.panelPadding.top + titleBlockHeight + 10, 16, 22, 400, colors.urlColor, 'data-part="imessage-description-text" aria-label="iMessage Description"'),
    siteName: buildTextModel(metadata.siteNameLines, layout.contentColumnX, metadata.siteNameLines[0] ? resolveBottomAlignedY(metadata.siteNameLines[0], metaRowBottomY, { fontSize: 17, fontWeight: 600 }) : 0, 17, 18, 600, colors.urlColor, 'data-part="imessage-site-name-text" aria-label="iMessage Site Name"'),
    date: buildTextModel(metadata.dateLines, metadata.dateX, metadata.dateLines[0] ? resolveBottomAlignedY(metadata.dateLines[0], metaRowBottomY, { fontSize: 16, fontWeight: 500 }) : 0, 16, 18, 500, colors.urlColor, 'data-part="imessage-date-text" aria-label="iMessage Published Date"'),
    domain: buildTextModel(metadata.domainLines, layout.contentColumnX, metadata.domainLines[0] ? resolveBottomAlignedY(metadata.domainLines[0], domainBottomY, { fontSize: 16, fontWeight: 500 }) : 0, 16, 18, 500, colors.urlColor, 'data-part="imessage-domain-text" aria-label="iMessage Domain Label"'),
  };
}
