...
function chooseCandidateColor(palette, backgroundColor, excluded = []) {
  ...
  return palette
    .map((color) => {
      ...
      return { color: hex, score: (contrastRatio(hex, backgroundColor) * 2) + (hueDistance(hsl.h, backgroundHsl.h) / 180) + hsl.s };
    })
...
export function resolveSmartPreviewColorsFromPalette({ dominant, palette = [] }) {
  ...
  const urlColor = urlCandidate ? adjustColorLightnessForContrast(urlCandidate, backgroundColor, 3) : deriveUrlColor(titleColor, backgroundColor);
