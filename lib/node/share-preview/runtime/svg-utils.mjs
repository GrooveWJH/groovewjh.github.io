export const SVG_FONT_STACK = "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'PingFang SC', 'Helvetica Neue', sans-serif";
...
function estimateTextMetrics(text, { fontSize, fontWeight }) {
  ...
  const hasAscii = /[\u0000-\u00ff]/.test(String(text || ""));

  return {
    width: estimateTextWidth(text, { fontSize: safeFontSize, fontWeight }),
    ascent: safeFontSize * (hasAscii ? 0.74 : 0.82),
    descent: safeFontSize * (hasAscii ? 0.22 : 0.18),
  };
}
