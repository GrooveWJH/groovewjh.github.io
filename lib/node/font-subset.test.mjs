...
test('resolvePyftsubsetCommand falls back to python3 module execution', () => {
  const command = resolvePyftsubsetCommand((commandName) => (
    commandName === 'python3-fonttools-subset'
  ));
...
test('buildDevelopmentFontCssText references original source fonts for dev builds', () => {
  ...
  assert.match(cssText, /src: url\(\"\/assets\/fonts\/NotoSerifCJKsc-Regular\.otf\"\) format\(\"opentype\"\);[\s\S]*unicode-range: U\+0009-000D, U\+0020-007E;/);
  assert.match(cssText, /src: local\("KaiTi"\), local\("Kaiti SC"\), local\("STKaiti"\), local\("Kai"\), local\("LXGW WenKai"\), url\(\"\/assets\/fonts\/Kaiti\.ttf\"\) format\(\"truetype\"\);[\s\S]*unicode-range: U\+3000-303F, U\+3400-4DBF, U\+4E00-9FFF, U\+FF00-FFEF;/);
});
...
test('planFontArtifactBuild cache hit when manifest and files match', () => {
  ...
  const subsetDescriptors = [
    {
      ...
      sourceHash: 'source-a',
      textHash: 'text-a',
      outputRelPath: 'fonts/noto-serif-sc-site-cjk-400.woff2',
      fingerprint: 'fp-a',
    },
  ];
  ...
  const plan = planFontArtifactBuild({
    ...
    previousManifest: {
      files: {
        'fonts/noto-serif-sc-site-cjk-400.woff2': { fingerprint: 'fp-a' },
      },
  ...
  const plan = planFontArtifactBuild({
    ...
    previousManifest: {
      files: {
        ...
        'fonts/noto-serif-sc-site-cjk-400.woff2': { fingerprint: 'fp-2' },
      },
...
test('planFontArtifactBuild rebuilds missing files even if manifest matches', () => {
  const subsetDescriptors = [
    {
      ...
      textHash: 'kai-text',
      outputRelPath: 'fonts/site-kai-site-cjk-400.woff2',
      fingerprint: 'kai-fp',
    },
  ];
  ...
  const plan = planFontArtifactBuild({
    ...
    previousManifest: {
      files: {
        'fonts/site-kai-site-cjk-400.woff2': { fingerprint: 'kai-fp' },
      },
      css: { fingerprint: 'css-fp' },
    },
    artifactExists: (outputRelPath) => outputRelPath !== 'fonts/site-kai-site-cjk-400.woff2',
    reuseCache: true,
    sourceFingerprints: [
      {
