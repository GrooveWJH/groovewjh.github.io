...
3. 进入仓库根目录（也就是包含 `package.json` 的目录），然后运行构建命令。常用命令如下：
  ...
  - `npm run build:force` ：强制刷新所有编译与复制任务；
  - `npm run build:fast` ：使用 `--jobs 4` 并发构建；
  - `npm run build:preview` ：输出到 `_site-preview/`，方便预览效果；
  - `npm run build:full` ：等价于 `--force --jobs 4`。
4. 在本地启动 Live Server 进行预览，或者将生成的站点页面部署到云端。

目前，`--force` 指令只会跳过未被修改的素材，而所有通过 `index.typ` 编写的页面都会被重新编译。这主要是出于“上一篇/下一篇”功能的考量，在未来可能会进行优化。
...
#table(
  ...
  [`-f` 或 `--force`], [强制刷新所有编译与复制任务，不再沿用旧结果],
