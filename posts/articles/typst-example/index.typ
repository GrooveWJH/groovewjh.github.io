...
#define MAX_LIMIT 42 // 测试宏定义和单行注释
...
void printValues(const std::vector<T>& vec) {
    for(size_t i = 0; i < vec.size(); ++i) {
        // 测试转义字符、字符串和输出流
        std::cout << \"Item \" << i << \":\t\" << vec[i] << \"\n\";
    }
}

int main() {
    std::string test_name = \"Highlight Test\";
    std::vector<double> numbers = {3.14, 2.718, 1.618}; // 测试浮点数
...
// 测试单行注释与自定义函数定义
#let highlight-test(title, body) = {
  align(center)[
    *#title*
    #v(1em)
  ]
  box(fill: luma(240), inset: 8pt, radius: 4pt)[
    #body
  ]
}

= Section One: Math & Symbols

/* 测试段落文本和内联/多行数学公式 */
Here is an inline equation $E = m c^2$ and a block equation:
$ integral_0^infty e^(-x^2) dx = sqrt(pi)/2 $

#highlight-test("Renderer Check")[
  - *Bold* and _Italic_ text.
  - A raw string: `"Hello, Typst!"`
  - Number formatting: `42.0` or `#calc.pi`.
]
```
